...
def am_stringchunk(data,pc):
    ...
    if int(ApUtils.printhex(StyleCount),16) != 0:
        ...
        for j in range(int(ApUtils.printhex(StyleCount),16)):
            StyleOffset = ApUtils.little_endian(data[pc + 28+int(ApUtils.printhex(StringCount),16)*4+j*4:pc + 28+int(ApUtils.printhex(StringCount),16)*4+j*4+4])
    ...
    if config.get_value('XMLFLAG') == 0:
        for index in config.global_list['STRINGOFFSETS']:
            ...
            #utf-8编码
            try:
                string = ApUtils.read_asc(data[str_index + 2:str_index + 2 + str_len * 2]).decode("utf-8")
            except:
                try:
                    string = ApUtils.read_asc(data[str_index + 2:str_index + 2 + str_len * 2]).decode("utf-16")
                except:
        ...
        for index in config.global_list['STRINGOFFSETS']:
            ...
            try:
                ...
                try:
                    string = ApUtils.read_asc(data[str_index + 2:str_index + 2 + str_len ]).decode("utf-16")
                except:
                    string = "String resolve error !!"
            config.set_value('STRINGCHUNKEND',str_index  + str_len +2 +1)
            ApUtils.apprint(" [StringPool]["+str(count)+"]"+string)
            config.global_list['STRINGPOOL'].append(string)
            count += 1
            config.set_value('LASTSTRINGLEN',str_len)
...
def am_resourceidchunk(data,pc):
    ...
    ResourcesId_num = (int(ApUtils.printhex(ChunkSize),16)-8)/4
    ...
    config.set_value('RESOURCEIDCHUNKEND',config.get_value('RESOURCEIDCHUNKSTAR')+int(ApUtils.printhex(ChunkSize),16))
...
def am_tagchunk_star(data,pc):
    ...
    if ApUtils.printhex(ChunkType) == "0x00100102":
        ...
        ApUtils.apprint(" [startTag"+str(config.get_value('TAGCOUNTSTAR'))+"]ChunkType is : " + ApUtils.printhex(ChunkType))
    else:
    ...
    # Tag Chunk的大小（4）
    ChunkSize = ApUtils.little_endian(data[pc + 4:pc + 8])
    ApUtils.apprint(" [startTag"+str(config.get_value('TAGCOUNTSTAR'))+"]ChunkSize is : " + ApUtils.printhex(ChunkSize))

    # Line Number 行号（4）
    LineNumber = ApUtils.little_endian(data[pc + 8:pc + 12])
    ApUtils.apprint(" [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]LineNumber is : " + ApUtils.printhex(LineNumber))
    ...
    if int(ApUtils.printhex(NameSpcae),16) <= len(config.global_list['STRINGPOOL']) :
        ApUtils.apprint(" [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]NameSpace is : " + config.global_list['STRINGPOOL'][int(ApUtils.printhex(NameSpcae),16)])
    else:
    ...
    ApUtils.apprint(" [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute Count is : " + ApUtils.printhex(AttributeCount))
    ...
    for i in range(int(ApUtils.printhex(AttributeCount),16)):
        ...
        att_data = ApUtils.little_endian(data[pc + 36 + i * 5 * 4 + 4 + 4+4+4:pc + 36 + i * 5 * 4 + 4 + 4 + 4+4+4])
        #打印
        ApUtils.apprint(" ----------------------------------------------")
        if int(ApUtils.printhex(att_namespace), 16) <= len(config.global_list['STRINGPOOL']):
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]NameSpace is : " + config.global_list['STRINGPOOL'][int(ApUtils.printhex(att_namespace), 16)])
        else:
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]NameSpace is null ")
        ...
        if int(ApUtils.printhex(att_name), 16) <= len(config.global_list['STRINGPOOL']):
            ...
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Name is : " + attr_name)
            #修改application name
            if NameStr == "application":
                if attr_name == "name":
            ...
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Name is null ")

        if int(ApUtils.printhex(att_valuestr), 16) <= len(config.global_list['STRINGPOOL']):
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Valus String is : " + config.global_list['STRINGPOOL'][int(ApUtils.printhex(att_valuestr), 16)])
        else:
            ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Valus String is null ")

        type = ApUtils.getAttrType(int(ApUtils.printhex(att_type),16)>>24)
        ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Type is : " + type)

        dataAtt = ApUtils.getAttrData(int(ApUtils.printhex(att_type),16)>>24,att_data)
        ApUtils.apprint("     [startTag" + str(config.get_value('TAGCOUNTSTAR')) + "]Attribute[" + str(i) + "]Data is : " + dataAtt)
    # 修改结束指针
...
def am_tagchunk_end(data,pc):
    ...
    if ApUtils.printhex(ChunkType) == "0x00100103":
        ...
        ApUtils.apprint(" [endTag" + str(config.get_value('TAGCOUNTEND')) + "]ChunkType is : " + ApUtils.printhex(ChunkType))
    else:
    ...
    # Tag Chunk的大小（4）
    ChunkSize = ApUtils.little_endian(data[pc + 4:pc + 8])
    ApUtils.apprint(" [endTag"+str(config.get_value('TAGCOUNTEND'))+"]ChunkSize is : " + ApUtils.printhex(ChunkSize))

    # Line Number 行号（4）
    LineNumber = ApUtils.little_endian(data[pc + 8:pc + 12])
    ApUtils.apprint(" [endTag" + str(config.get_value('TAGCOUNTEND')) + "]LineNumber is : " + ApUtils.printhex(LineNumber))
    ...
    NameSpcae = ApUtils.little_endian(data[pc + 16:pc + 20])
    if int(ApUtils.printhex(NameSpcae),16) <= len(config.global_list['STRINGPOOL']) :
        ApUtils.apprint(" [endTag" + str(config.get_value('TAGCOUNTEND')) + "]NameSpace is : " + config.global_list['STRINGPOOL'][int(ApUtils.printhex(NameSpcae),16)])
    else:
