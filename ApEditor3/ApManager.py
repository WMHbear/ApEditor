...
from modules.ApEditor import config, ApUtils, ApResolver
...
def changeApplication(source_filepath, target_filepath, application_name):
    ...
    old_filesize = ApUtils.printhex(
        ApUtils.little_endian(data_hex[config.get_value('FILSIZEINDEX'):config.get_value('FILSIZEINDEX') + 4]))
    ...
    # String chunk size
    old_stringchunk_size = ApUtils.printhex(ApUtils.little_endian(
        data_hex[config.get_value('STRINGCHUNKSIZEINDEX'):config.get_value('STRINGCHUNKSIZEINDEX') + 4]))
    ...
    # String chunk count
    old_stringchunk_count = ApUtils.printhex(ApUtils.little_endian(
        data_hex[config.get_value('STRINGCHUNKCOUNTINDEX'):config.get_value('STRINGCHUNKCOUNTINDEX') + 4]))
    ...
    # Stringpool off set
    old_stringpool_offset = ApUtils.printhex(ApUtils.little_endian(
        data_hex[config.get_value('STRINGPOOLOFFSETINDEX'):config.get_value('STRINGPOOLOFFSETINDEX') + 4]))
    ...
    if config.get_value('APPLICATIONNAMEFLAG') == 1:
        ...
        # name不换
        cp_data.extend(
            data_hex[config.get_value('APPLICATIONNAMEINDEX') + 4:config.get_value('APPLICATIONNAMEINDEX') + 4 + 4])
        # valuesstr换,这里是下标，从0开始，因此-1
        cp_data.extend(ApUtils.encodehex(new_stringchunk_count - 1))
        # type不换
        cp_data.extend(data_hex[config.get_value('APPLICATIONNAMEINDEX') + 4 + 4 + 4:config.get_value(
            'APPLICATIONNAMEINDEX') + 4 + 4 + 4 + 4])
        # data换
        cp_data.extend(ApUtils.encodehex(new_stringchunk_count - 1))
